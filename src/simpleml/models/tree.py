...
class DecisionTreeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _best_split(self, X: np.ndarray, y: np.ndarray) -> Tuple[Optional[int], Optional[float]]:
        ...
        for feature in range(self.n_features_):  # type: ignore
    ...
    def _traverse_tree(self, x: np.ndarray, node: Node) -> float:
        ...
        if x[node.feature] <= node.threshold:
            return self._traverse_tree(x, node.left) if node.left else 0.0
        else:
            return self._traverse_tree(x, node.right) if node.right else 0.0
