...
from ..core.base import BaseEstimator, ClusterMixin
...
class KMeans(BaseEstimator, ClusterMixin):
    ...
    def _initialize_centroids(self, X: np.ndarray, rng: np.random.RandomState) -> np.ndarray:
        """Initialize cluster centroids."""
        n_samples = X.shape[0]
        n_clusters_actual = min(self.n_clusters, n_samples)
        if n_clusters_actual < self.n_clusters:
            raise ValueError(
...
class DBSCAN(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X: np.ndarray, y=None) -> 'DBSCAN':
        ...
        # Store core points and their labels for use in predict
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        Each new point is assigned the label of the nearest core point
        within eps distance. Points with no core point nearby are labelled -1.
        ...
        -------
        labels : ndarray of shape (n_samples,)
            Cluster label for each point, or -1 if it is noise.
        """
        X = np.asarray(X)
        labels = np.full(X.shape[0], -1, dtype=int)

        if not self.core_sample_indices_:
            return labels

        core_points = self.X_train_[self.core_sample_indices_]
        core_labels = self.labels_[self.core_sample_indices_]

        for i, x in enumerate(X):
            distances = np.sqrt(np.sum((core_points - x) ** 2, axis=1))
            within_eps = np.where(distances <= self.eps)[0]
            if len(within_eps) > 0:
                nearest = within_eps[np.argmin(distances[within_eps])]
                labels[i] = core_labels[nearest]
