...
class LinearSVC(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> 'LinearSVC':
        ...
        y[y == unique_classes[0]] = -1.0
        y[y == unique_classes[1]] = 1.0
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        X = np.asarray(X, dtype=np.float64)
        raw = np.sign(X @ self.coef_ + self.intercept_)  # -1.0 or +1.0
        result = np.where(raw <= 0, self.classes_[0], self.classes_[1])
        return result
