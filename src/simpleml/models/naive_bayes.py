...
from ..core.base import BaseEstimator, ClassifierMixin
...
class GaussianNaiveBayes(BaseEstimator, ClassifierMixin):
    ...
    def _calculate_likelihood(self, x: np.ndarray, theta: np.ndarray, sigma: np.ndarray) -> np.ndarray:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        for idx, x in enumerate(X):
            ...
            for i, c in enumerate(self.classes_):  # type: ignore
                prior = np.log(self.class_prior_[i])  # type: ignore
                posterior = np.sum(np.log(self._calculate_likelihood(
                    x, self.theta_[i, :], self.sigma_[i, :] + 1e-10  # type: ignore
                )))
                posteriors.append(prior + posterior)

            y_pred[idx] = self.classes_[np.argmax(posteriors)]  # type: ignore
...
class MultinomialNaiveBayes(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        X = np.asarray(X)
        return self.classes_[np.argmax(  # type: ignore
            self.class_log_prior_ + np.dot(X, self.feature_log_prob_.T),  # type: ignore
            axis=1
        )]
